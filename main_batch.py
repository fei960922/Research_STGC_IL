...
def main():
    ...
    parser.add_argument('-batch_version', type=int, default=-1)
    ...
    train_img, train_label = loadActionDemo(opt.data_path, 500)
    ...
    if opt.batch_version == 1:
        '''
        Batch training 20180305
        Training V2.0, all cold start, scale 0-1 
        step_size = [0.001, 0.0015, 0.002, 0.003, 0.004]
        '''
        for step_size in [0.0015, 0.002, 0.003, 0.004]:
            tf.reset_default_graph()
            print("Start training step_size = %.2f" % step_size)
            opt.step_size = step_size
            opt.category = "BatchTrain_20180305-stepsize%.4f" % step_size
            with tf.Session() as sess:
                model = STGConvnet(sess, opt)
                model.train(train_img, train_label)

    if opt.batch_version == 2:
        '''
        Batch training 20180305
        Training V2.0, all cold start, scale 0-1 
        step_size = [0.005, 0.01, 0.05, 0.1, 0.3]
        '''
        for step_size in [0.01, 0.05, 0.1, 0.3]:
            tf.reset_default_graph()
            print("Start training step_size = %.2f" % step_size)
            opt.step_size = step_size
            opt.category = "BatchTrain_20180305-stepsize%.4f" % step_size
    ...
    if opt.batch_version == 3:
        '''
        Batch training awaiting...
        Training V2.0, all cold start, scale 0-1 
        step_size = [0.001, 0.0015, 0.002, 0.003, 0.004]
        '''
        for step_size in [0.0015, 0.002, 0.003, 0.004]:
